...
if __name__ == '__main__':
    ...
    for i, l in enumerate(layers):
        ...
        txt = "yolov2.bias{0:d}.b.data = dat[{1:d}:{2:d}]".format(i+1, offset, offset+out_ch)
        offset+=out_ch
        exec(txt)
    
        # load bn(BatchNormalization.gammaはout_chと同じサイズ)
        txt = "yolov2.bn{0:d}.gamma.data = dat[{1:d}:{2:d}]".format(i+1, offset, offset+out_ch)
        offset+=out_ch
        exec(txt)
    
        # (BatchNormalization.avg_meanはout_chと同じサイズ)
        txt = "yolov2.bn{}.avg_mean = dat[{}:{}]".format(i+1, offset, offset+out_ch)
        offset+=out_ch
        exec(txt)
    
        # (BatchNormalization.avg_varはout_chと同じサイズ)
        txt = "yolov2.bn{}.avg_var = dat[{}:{}]".format(i+1, offset, offset+out_ch)
        offset+=out_ch
        exec(txt)
    
        # load convolution weight(Convolution2D.Wは、outch * in_ch * フィルタサイズ。これを(out_ch, in_ch, 3, 3)にreshapeする)
        txt = "yolov2.conv{0:d}.W.data = dat[{1:d}:{2:d}].reshape({3:d}, {4:d}, {5:d}, {6:d})".format(i+1, offset, offset+(out_ch*in_ch*ksize*ksize), out_ch, in_ch, ksize, ksize)
        print(txt)    
    ...
    txt = "yolov2.bias{}.b.data = dat[{}:{}]".format(i+2, offset, offset+out_ch)
    offset+=out_ch
    exec(txt)
    
    txt = "yolov2.conv{}.W.data = dat[{}:{}].reshape({}, {}, {}, {})".format(i+2, offset, offset+(out_ch*in_ch*ksize*ksize), out_ch, in_ch, ksize, ksize)
    offset+=out_ch*in_ch*ksize*ksize
    exec(txt)
    print(i+2, offset)
